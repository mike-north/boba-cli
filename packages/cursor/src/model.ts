...
export class CursorModel {
  ...
  tickBlink(): [CursorModel, Cmd<BlinkMsg>] {
    ...
    const cmd: Cmd<BlinkMsg> = tick(speed, (time: Date) => new BlinkMsg(id, nextTag, time));
    return [next, cmd];
  }
  ...
  withMode(mode: CursorMode): [CursorModel, Cmd<Msg>] {
    const bounded =
      mode === CursorMode.Blink || mode === CursorMode.Static || mode === CursorMode.Hidden
        ? mode
        : CursorMode.Blink;

    const blink = bounded === CursorMode.Hidden || !this.#focus ? true : this.#blink;
